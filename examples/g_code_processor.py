...
class GCodeProcessor:
    ...
    def build_new_point(self, cmd: list):
        ...
        for i, value in enumerate(xyz_val):
            if np.isnan(value):
                self.new_point[i] = self.last_point[i] - self.offset[0][i]
            else:
                self.new_point[i] = value
        ...
        self.new_point, self.new_or = p.multiplyTransforms(
            self.new_point, orientation, self.offset[0], orientation_offset)
