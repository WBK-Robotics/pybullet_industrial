...
class PathPlannerGUI:
    ...
    def create_widgets(self) -> None:
        ...
        for i, joint_name in enumerate(self.joint_order):
            ...
            tk.Button(joints_frame, text="+", command=lambda i=i: self.increment_joint(i), width=2)\
                .grid(row=i, column=1, padx=2, pady=1)
            tk.Button(joints_frame, text="-", command=lambda i=i: self.decrement_joint(i), width=2)\
        ...
        for i, label in enumerate(obs_labels):
            ...
            tk.Button(obstacle_frame, text="+", command=lambda i=i: self.increment_obstacle(i), width=2)\
                .grid(row=i+1, column=1, padx=2, pady=1)
            tk.Button(obstacle_frame, text="-", command=lambda i=i: self.decrement_obstacle(i), width=2)\
