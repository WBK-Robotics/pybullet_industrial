...
A path contains a 3 dimensional array whith each dimension containing subsequent positions.
...
In this case the path contains 100 points. A circle can be generated in a similar manner. 
After the paths are created, they are drawn. The default colour of a path is blue.
The result looks like this:

.. image:: images/box_path.png

You can generate a path between two points by utilizing interpolation functions.
Here the colour of the paths is prescribed.
...
In order to make a robot follow a path and print particles, the simulation must be set up first.

In order to integrate a robot into a simulation its URDF file must be loaded first.
Together with a position and an orientation it can be loaded in the GUI.
For the extruder and the milling tool respectively porperties must be defined as well.
For both the extruder and the remover a maximum distance, an opening angle and the number of rays need to be specified.
With these parameters the tool can determine wether other objects are close enough to interact with.
...
For each position in the path the tools are translated and extrude respectively remove particles.
