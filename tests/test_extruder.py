...
class TestExtruder(unittest.TestCase):
    ...
    def test_extrusion_distance(self):
        ...
        not_extruding_beyond_zero = (test_path_zero[:,2:] < 10**-3).all() #discard first printed particles as they sit on the surface and not each other
    ...
    def test_material_distribution(self):
        ...
        """
        target_position = np.array([1.9, 0.3,0.6]) 
        extruder.change_extruder_properties({'maximum distance':0.7,'opening angle':np.pi/6,'number of rays':300})
        ...
        extruder.change_extruder_properties({'maximum distance':0.5,'opening angle':0,'number of rays':1})
        ...
        mean_is_uniform = (np.abs(np.mean(distance_from_center_list)-cone_radius/2) <= 10**-1)
        var_is_uniform =  (np.var(distance_from_center_list)-1/12*cone_radius**2 <= 10**-1)
