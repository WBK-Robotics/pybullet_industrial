...
if __name__ == "__main__":
    ...
    with open(robot_view_path, encoding='utf-8') as f:
        gcode_input = f.read()
    raw_g_code = g_code_processor.read_g_code(gcode_input)

    g_code_simplifier = GCodeSimplifier(raw_g_code)

    simpflified_g_code = g_code_simplifier.simplify(0.1)
    simpflified_g_code = g_code_simplifier.transform_g_code(
        simpflified_g_code, round_dec=2)
    simpflified_g_code_path = os.path.join(
        working_dir, 'g_codes', 'g_code_simplified.txt')
    g_code_logger.write_g_code(simpflified_g_code, simpflified_g_code_path)
